...
router.get('/accounts', authenticateToken, async (req, res) => {
  try {
    const response = await makeRabobankAPICall(
      'GET',
      '/v1/accounts',
      req.user.rabobankToken
    );

    // Transform Rabobank response to our format
    ...
    handleRabobankAPIError(error, res);
  }
});
...
router.get('/accounts/:accountId/transactions', authenticateToken, async (req, res) => {
  try {
    ...
    // Transform Rabobank transactions to our format
...
router.post('/sync', authenticateToken, async (req, res) => {
  try {
    ...
    // Get current balance
    const balanceResponse = await makeRabobankAPICall(
      'GET',
      `/v1/accounts/${accountId}/balances`,
      req.user.rabobankToken
    );
...
/**
 * Helper function: Make authenticated API call to Rabobank
 */
async function makeRabobankAPICall(method, endpoint, accessToken, data = null) {
  const config = {
    method,
    url: `${process.env.RABOBANK_API_BASE_URL}${endpoint}`,
    headers: {
...
/**
 * Helper function: Handle Rabobank API errors
 */
function handleRabobankAPIError(error, res) {
  if (error.response) {
    // Rabobank API returned an error
    const status = error.response.status;
    const message = error.response.data?.error_description || error.response.data?.message || 'API Error';
    
    if (status === 401) {
      res.status(401).json({ error: 'Rabobank token expired', needsRefresh: true });
    } else if (status === 403) {
      res.status(403).json({ error: 'Access denied by Rabobank', message });
    } else if (status === 429) {
      res.status(429).json({ error: 'Rate limit exceeded', message });
    } else {
      res.status(status).json({ error: 'Rabobank API Error', message });
    }
  } else if (error.request) {
    // Network error
    res.status(503).json({ error: 'Unable to reach Rabobank API', message: 'Network error' });
  } else {
    // Other error
